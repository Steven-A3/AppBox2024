...
typedef NS_ENUM(NSUInteger, CDEMergingPhase) {
    ...
    CDEMergingPhaseFileContentsMerge
};
...
typedef NS_OPTIONS(NSUInteger, CDEMergeOptions) {
 ...
 */
- (nullable instancetype)initWithEnsembleIdentifier:(nonnull NSString *)identifier persistentStoreURL:(nonnull NSURL *)storeURL managedObjectModelURL:(nonnull NSURL *)modelURL cloudFileSystem:(nonnull id <CDECloudFileSystem>)cloudFileSystem;
...
/**
 Begins merging files from a cloud directory with a local directory. Any files absent locally are downloaded and added to the directory, and files missing in the cloud are uploaded.
 
 No check is made of file content. If a file has the same name, it is assumed to have the same content. This makes the method inappropriate for mutable files.
 
 Files are stored together with the ensemble data in the cloud. Removing the ensemble data will also remove the files.
 
 Deletion of files is not tracked. If a file is deleted from either the cloud, or locally, it is very likely to reappear on the next merge, because this method does not keep track of what files existed prior to the call.
 
 @param localDir Local directory that contains the files to sync. Should be absolute path.
 @param cloudPath Remote directory that contains the files. Should be relative path. e.g. "files", will be created inside ensemble directory.
 @param completion A block that is executed upon completion of merging, whether successful or not. The block is passed nil upon a successful merge, and an NSError otherwise.
 @param progressBlock A block that is called repeatedly to update the progress of the merge.
 */
- (void)mergeFileContentOfLocalDirectory:(nonnull NSString *)localDir withCloudDirectory:(nonnull NSString *)cloudPath progress:(nullable CDEProgressBlock)progressBlock completion:(nullable CDECompletionBlock)completion;

/**
 Rather than repeatedly calling the `mergeFileContentOfLocalDirectory:...` method manually, you can use this method so that the ensemble always includes it. Call it for as many directories as you need. The ensemble will merge each one each time you call `mergeWithCompletion:` on the ensemble.
 
 In short, this is a way to make sync of a folder of files part of the standard sync.
 
 @param localDir Local directory that contains the files to sync. Should be absolute path.
 @param cloudPath Remote directory that contains the files. Should be relative path. e.g. "files", will be created inside ensemble directory.
 **/
- (void)alwaysMergeFileContentInLocalDirectory:(nonnull NSString *)localDir withCloudDirectory:(nonnull NSString *)cloudPath;

/**
 Deletes local file content, and cloud file content for the filenames passed in. This can be used to clean up file content that is no longer needed. It is no guarantee that
 the files will not reappear, because other devices may reupload them.
 
 If a file is not present locally or remotely, it continues without error, just ignoring the missing file.
 
 If the ensemble is not leeched, only the local files will be deleted. 
 
 @param filenames The names of the files to delete.
 @param localDir Local directory that contains the files to delete. Should be absolute path.
 @param cloudDir Remote directory that contains the files to delete. Should be relative path. e.g. "files", will be created inside ensemble directory.
 */
- (void)deleteFiles:(nonnull NSArray *)filenames inLocalDirectory:(nonnull NSString *)localDir cloudDirectory:(nonnull NSString *)cloudDir completion:(nullable CDECompletionBlock)completion;
