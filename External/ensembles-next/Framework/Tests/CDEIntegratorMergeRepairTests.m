...
- (void)testRelationshipUpdateGeneratesObjectChange
{
    ...
    [self.eventStore.managedObjectContext performBlockAndWait:^{
        ...
        XCTAssertEqual(parentChange.propertyChangeValues.count, (NSUInteger)1, @"Wrong property change values on parent. (Only stores one-to-one relationship from Child side.)");
