...
- (void)testConsolidatingWithMultipleEmptyBaselines
{
    ...
    [self addBaselineEventsForStoreId:@"234" globalCounts:@[@(0)] revisions:@[@(0)]];
    __block NSString *mergedBaselineID = nil;
    [context performBlockAndWait:^{
        mergedBaselineID = [mergedBaseline.uniqueIdentifier copy];
    }];
...
- (void)testConsolidatingMultipleBaselinesWithMultipleStoresKeepsMostRecent
{
    ...
    __block CDEStoreModificationEvent *mostRecentBaseline = baselines.lastObject;
    NSManagedObjectID *mostRecentBaselineID = mostRecentBaseline.objectID;
    ...
    [consolidator consolidateBaselineWithCompletion:^(NSError *error) {
        ...
        [context performBlock:^{
            ...
            CDEStoreModificationEvent *event = events.lastObject;
            mostRecentBaseline = (id)[context objectWithID:mostRecentBaselineID];
...
- (void)testBaselineRevisionsWhenMergingConcurrentBaselines
{
    ...
    [consolidator consolidateBaselineWithCompletion:^(NSError *error) {
        ...
        [context performBlock:^{
            ...
            XCTAssertEqual(event.eventRevision.revisionNumber, (CDERevisionNumber)11, @"Wrong revision number for 234");
            ...
            NSSet *set = [NSSet setWithObjects:rev1, nil];
...
- (void)testMergingConcurrentBaselinesWithManyObjectChanges
{
    CDEStoreModificationEvent *baseline0 = [[self addBaselineEventsForStoreId:@"123" globalCounts:@[@(10)] revisions:@[@(10)]] lastObject];
    CDEStoreModificationEvent *baseline1 = [[self addBaselineEventsForStoreId:@"234" globalCounts:@[@(20)] revisions:@[@(10)]] lastObject];
    [context performBlockAndWait:^{
        for (NSUInteger i = 0; i < 1000; i++) {
            NSString *globalIdString = [[NSProcessInfo processInfo] globallyUniqueString];
    ...
    [consolidator consolidateBaselineWithCompletion:^(NSError *error) {
        ...
        [context performBlock:^{
            ...
            NSSet *changes = event.objectChanges;
            XCTAssertEqual(changes.count, (NSUInteger)1000, @"Wrong number of changes");
            
            NSSet *dates = [changes valueForKeyPath:@"propertyChangeValues.value"];
            XCTAssertEqual(dates.count, (NSUInteger)1, @"Wrong number of dates");
