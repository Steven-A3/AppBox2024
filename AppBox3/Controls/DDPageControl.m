...
//				CGContextFillEllipseInRect(context, CGRectInset(dotRect, 1.2f, 1.2f)) ;

				CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
				CGFloat locations[] = { 0.0, 1.0 };

				NSArray *colors = [NSArray arrayWithObjects:
						(__bridge id)[UIColor colorWithRed:123.0f/255.0f green:123.0f/255.0f blue:123.0f/255.0f alpha:1.0f].CGColor,
						(__bridge id)[UIColor colorWithRed:76.0f/255.0f green:76.0f/255.0f blue:76.0f/255.0f alpha:1.0f].CGColor,
						nil];

				CGGradientRef gradient = CGGradientCreateWithColors(colorSpace, (__bridge CFArrayRef) colors, locations);

				CGRect drawingRect = CGRectInset(dotRect, 1.2f, 1.2f);

				CGPoint startPoint = CGPointMake(CGRectGetMidX(drawingRect), CGRectGetMinY(drawingRect) - 1.0f);
				CGPoint endPoint = CGPointMake(CGRectGetMidX(drawingRect), CGRectGetMaxY(drawingRect) + 1.0f);

				CGContextSaveGState(context);
				CGContextBeginPath(context);
				CGContextAddArc(context,
						CGRectGetMidX(dotRect),
						CGRectGetMidY(dotRect),
						CGRectGetWidth(dotRect)/2.0f - 1.0f,
						DegreesToRadians(0), DegreesToRadians(360), 0);
