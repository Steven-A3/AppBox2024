...
{
    if ([[CoreDataStack shared] coreDataReady] && [self shouldShowHelpView]) {
        [self showInstructionView];
    }
}

- (BOOL)shouldShowHelpView {
	return [[CoreDataStack shared] coreDataReady] && ![[A3UserDefaults standardUserDefaults] boolForKey:A3V3InstructionDidShowForWalletCategoryView];
}
        ...
        if ([section[indexPath.row] isKindOfClass:[WalletItem_ class]]) {
			...
			[fieldItems enumerateObjectsUsingBlock:^(WalletFieldItem_ *fieldItem, NSUInteger idx, BOOL *stop) {
				...
				if ([fieldItem.hasImage boolValue]) {
                    ...
                    NSURL *imageURL = [fieldItem imageURL];
                    if ([fileManager fileExistsAtPath:imageURL.path]) {
                        result = [fileManager removeItemAtURL:imageURL error:NULL];
						NSAssert(result, @"result");
					}
				} else {
                    ...
                    NSURL *videoURL = [fieldItem videoURL];
                    if ([fileManager fileExistsAtPath:videoURL.path]) {
                        result = [fileManager removeItemAtURL:videoURL error:NULL];
						NSAssert(result, @"result");
					}
				}
			}];
