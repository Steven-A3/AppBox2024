//
//  A3CurrencyPickerStyleViewController_OLD.m
//  AppBox3
//
...
@interface A3CurrencyPickerStyleViewController_OLD ()
...
@implementation A3CurrencyPickerStyleViewController_OLD {
...
- (void)viewWillAppear:(BOOL)animated {
	...
	if ([self isMovingToParentViewController] || [self isBeingPresented]) {
		...
		dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
			[self setupBannerViewForAdUnitID:AdMobAdUnitIDCurrencyList keywords:@[@"Finance", @"Money", @"Shopping", @"Travel"] gender:kGADGenderUnknown];
		});
	} else {
		_favorites = nil;
...
- (void)updateChartViewFrom:(NSString *)from to:(NSString *)to {
	...
	if ([[A3AppDelegate instance].reachability isReachable]) {
		...
		__weak A3CurrencyPickerStyleViewController_OLD *weakself = self;
