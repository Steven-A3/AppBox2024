...
#import "A3DateCalcMainViewController.h"
...
@interface A3DateCalcMainViewController () <UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, UIPopoverControllerDelegate,
                                        A3DateKeyboardDelegate, A3DateCalcExcludeDelegate, A3DateCalcDurationDelegate, A3DateCalcHeaderViewDelegate, A3DateCalcEditEventDelegate>
...
@property (weak, nonatomic) IBOutlet UITextField *fromToTextField;
...
@implementation A3DateCalcMainViewController {
...
{
    self.headerView = [[[NSBundle mainBundle] loadNibNamed:@"A3DateCalcHeaderView"
                                                     owner:nil
    ...
    if (IS_IPHONE) {
        ...
        //self.headerView.frame = CGRectMake(0, 0, CGRectGetWidth(self.tableView.frame), IS_RETINA? 103.5 : 104);
        self.headerView.frame = CGRectMake(0, 0, CGRectGetWidth(self.tableView.frame), 104.0);
        
    } else {
        kCalculationString = @"CALCULATION";
        self.tableView.separatorInset = UIEdgeInsetsMake(0, 28.0, 0, 0);
        self.headerView.frame = CGRectMake(0, 0, CGRectGetWidth(self.tableView.frame), IS_RETINA? 157.5 : 158.0);
...
-(void)viewWillLayoutSubviews
{
    ...
    if (IS_LANDSCAPE) {
        _tableYOffset = self.navigationController.navigationBar.bounds.size.height + [[UIApplication sharedApplication] statusBarFrame].size.width;
        self.tableView.contentInset = UIEdgeInsetsMake(_tableYOffset, 0, 0, 0);
        self.tableView.contentOffset = CGPointMake(0.0, -_tableYOffset);
    } else {
        _tableYOffset = self.navigationController.navigationBar.bounds.size.height + [[UIApplication sharedApplication] statusBarFrame].size.height;
        self.tableView.contentInset = UIEdgeInsetsMake(_tableYOffset, 0, 0, 0);
        self.tableView.contentOffset = CGPointMake(0.0, -_tableYOffset);
    }
...
//            if (IS_IPHONE) {
//                A3NumberKeyboardViewController_iPhone * keyboard = [[A3NumberKeyboardViewController_iPhone alloc] initWithNibName:@"A3NumberKeyboardSimpleVC_iPhone" bundle:nil];
//                [keyboard.prevButton setImage:nil forState:UIControlStateNormal];
//                [keyboard.prevButton setTitle:@"Prev" forState:UIControlStateNormal];
//                [keyboard.nextButton setImage:nil forState:UIControlStateNormal];
//                [keyboard.nextButton setTitle:@"Next" forState:UIControlStateNormal];
//                
//                _simpleNormalNumberKeyboard = keyboard;
//            } else {
//                _simpleNormalNumberKeyboard = [[A3NumberKeyboardSimpleVC_iPad alloc] initWithNibName:@"A3NumberKeyboardSimpleVC_iPad" bundle:nil];
//                //_simpleNormalNumberKeyboard = [[A3NumberKeyboardViewController_iPad alloc] initWithNibName:@"A3NumberKeyboardViewController_iPad" bundle:nil];
//            }
        }
...
{
    NSLog(@"handleBigButton1");
}

- (void)handleBigButton2
{
    NSLog(@"handleBigButton2");
}
    ...
    if (bInit) {
        ...
        if (self.isAddSubMode) {
            ...
            [CATransaction setCompletionBlock:^{
                ...
                [_tableView insertSections:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(2, 2)] withRowAnimation:UITableViewRowAnimationFade];
                [_tableView endUpdates];
                [CATransaction commit];
            }];
            
            [_tableView beginUpdates];
            [_tableView deleteRowsAtIndexPaths:@[[NSIndexPath indexPathForRow:1 inSection:1]]
                              withRowAnimation:UITableViewRowAnimationFade];
            [_tableView deleteSections:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(2, 2)]
                      withRowAnimation:UITableViewRowAnimationFade];
            ...
            [CATransaction setCompletionBlock:^{
                ...
                [_tableView insertSections:[NSIndexSet indexSetWithIndex:2] withRowAnimation:UITableViewRowAnimationFade];
                [_tableView insertSections:[NSIndexSet indexSetWithIndex:3] withRowAnimation:UITableViewRowAnimationFade];
                self.tableView.allowsSelection = YES;
                [_tableView endUpdates];
            ...
            [_tableView deleteSections:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(2, 2)] withRowAnimation:UITableViewRowAnimationFade];
            [_tableView endUpdates];
            [CATransaction commit];
        }
    }
