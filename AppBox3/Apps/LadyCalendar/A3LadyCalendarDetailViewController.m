...
- (void)editAction:(id)sender
{
    ...
    viewCtrl.periodItem = [editingPeriod MR_inContext:[NSManagedObjectContext MR_rootSavingContext]];
...
- (void)editDetailItem:(UIButton *)button
{
	...
	viewCtrl.dataManager = self.dataManager;
    viewCtrl.isEditMode = YES;
    viewCtrl.periodItem = [item MR_inContext:[NSManagedObjectContext MR_rootSavingContext]];
	UINavigationController *navCtrl = [[UINavigationController alloc] initWithRootViewController:viewCtrl];
    navCtrl.modalPresentationStyle = UIModalPresentationCurrentContext;
    [self presentViewController:navCtrl animated:YES completion:nil];
}
